...
(() => {
  ...
  function isClaimable(tile, key, idx, settings2) {
    ...
    if (claim) {
      return claim.rd === "Yours" && !!settings2.ownClaimsAvailable;
    }
    return true;
  }
...
`;
  ...
  var SETTINGS_FIELDS = [
    ...
    { key: "ownClaimsAvailable", group: "Neighbours", label: "Treat Your Own Claims as Available", type: "checkbox" },
    {
      ...
      type: "checkbox",
      menu: true,
      hint: "Folds the panel down to its icon on any page other than the World Map \u2014 the only page where the scanner has map data to read."
    }
  ];
  ...
  function settingsMenuHtml(settings2) {
    ...
    const rows = fields.map((f) => `${fieldHtml(f, settings2)}${f.hint ? `<p class="sov-hint">${escapeHtml(f.hint)}</p>` : ""}`).join("");
...
licence and full copyright notice.">\u24D8</a></span></span></h2>
    ...
    root.querySelector(".sov-build").textContent = false ? "dev" : "1.1.2";
    ...
    function syncFocusRadiusHint() {
      ...
      focusForm.querySelector(".sov-radius-hint").textContent = `How far out sovereignty may be placed. Blank follows the claim radius in City Configuration, currently ${rClaim}.`;
      syncTownPicker();
    }
